---
title: "Writing API queries without a R wrapper: OMDb example"
output: html_document
---

**Main goal of this tutorial: illustrate how to use an API without a R wrapper using the OMDb API as example (Tip: opt for APIs with an R tutorial, unless the API documentation is very well done, like in this example)** 
...
Go to the OMDb website: https://www.omdbapi.com/ and click on the "API Key" tab to generate your API key. You can get a free key by entering your email. 
Set up your key in your R profile (recommended), as described in `geonames.Rmd`. Then, retrieve the key by running the following code:
...
What if no wrapper package exist? We have to write our own code and function to interact with the API!

First, we're going to examine the structure of API requests via the [Open Movie Database](https://www.omdbapi.com/). 

OMDb is very similar to IMDB, except it has a nice, simple API. We can go to the website, input some search parameters, and obtain both the JSON query and the response from it. 


### Constructing the API GET Request

The most challenging part of using web APIs is learning how to format your GET request URLs. While there are common architectures for such URLs, each API has its own unique quirks. For this reason, reviewing the API documentation is critical.
...
You can play around with the parameters on the [OMDB website](https://www.omdbapi.com/), and look at the resulting API call and the query you get back.
...
We want to convert the results from JSON format to something easier to work with: a data frame! For relatively simple API queries, one can use `as_tibble()` to convert the output to a data frame:
...
sharknados_df <- map_dfr(.x = sharknados, 
                         .f = omdb_api_slow, 
                         api_key = omdb_key)
sharknados_df
```
...
- Example developed starting from Benjamin's Soltoff "Computing for the Social Science" and from Rochelle Terman's [Collecting Data from the Web](https://plsc-31101.github.io/course/collecting-data-from-the-web.html)
