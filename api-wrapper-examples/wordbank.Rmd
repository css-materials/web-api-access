...
* [`wbstats`](https://github.com/gshs-ornl/wbstats): a wrapper for R for easy access to the API and returns data in a tidy format. A little outdated (last commit to the repo: 3 years ago).
...
```{r wb-cachelist}
# display only the top level of the object structure
...
```
Focus only on the indicators table:
...
Then, go back to R and do the same using the function `wb_search()`, be careful as this is case sensitive

```{r}
wb_search("Unemployment")
```
To match what you get here from the website, go back to the website `https://data.worldbank.org/indicator` and click on one indicator, then click on "Details" to see its ID, and compare it with what you get from the output above.
...
**NB: do not run the same APIs query code multiple times!** Run it once, and when you have collected your data, save them in a .`csv` file, and import them back (vs. interactively download them every time you run the script). 

Code to save and import back your data (assuming you are working in a `Rproject`, you do not need to specify the directory): 
